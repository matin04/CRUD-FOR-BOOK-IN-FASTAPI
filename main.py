...
@app.post("/create_book/", response_model=BookResponse, summary="Create new book")
...
@app.get("/list_book/", response_model=list[dict], summary="All Books")
...
async def deleted_book_endpoint(book_id:int):
    ...
    if not deleted:
        raise HTTPException(status_code=404, detail="BOOK NOT FOND")
    return deleted